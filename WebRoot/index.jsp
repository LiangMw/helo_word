...
<%@ taglib uri="http://www.holdsoft.com/tagext" prefix="hold" %>
	...
	<div id="layout">
		<div id="header">
			<div class="headerNav">
				...
				<ul class="nav">
					<!--<li id="switchEnvBox"><span>当前用户:</span>${loginUsername }</a>
						 <ul>
		...
		<div id="leftside">
			...
			<div id="sidebar">
				...
				<div class="accordion" fillSpace="sidebar">
					<c:forEach items="${lv1Menu}" var="item" varStatus="s">
						<div class="accordionHeader">
						<h2><span>Folder</span>${item.functionname }</h2>
						</div>
						<div class="accordionContent">
							<ul class="tree treeFolder">
								<c:forEach items="${item.selectLv2FunctionsByFatheridUserid(item.id,loginUserid,loginUser.username)}" var="sub" varStatus="s">
									<li><a href="${sub.url }"  target="navTab" rel="${sub.rel }" refresh="false">${sub.functionname }</a></li>
								</c:forEach>
							</ul>
						</div>	
					</c:forEach>
					 <%-- <c:forEach items="${lv1Menu}" var="item" varStatus="s">
						...
						<div class="accordionContent">
							...
							<c:forEach items="${item.selectLv2FunctionsByFatheridUserid(item.id,loginUserid,loginUser.username)}" var="subs" varStatus="ss">
								 <li><a href="#">${subs.functionname }</a> 
								 
									<ul>
										<c:forEach items="${subs.selectLv2FunctionsByFatheridUserid(subs.id,loginUserid,loginUser.username)}" var="sub" varStatus="s">
											<li><a href="${sub.url }"  target="navTab" rel="${sub.rel }" refresh="false">${sub.functionname }</a></li>
										</c:forEach>
									</ul>
								 </li> 
